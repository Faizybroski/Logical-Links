...
export function PricingSummary({
  subtotal, discount, taxRate, tax, total, currency = "AUD",
  amountPaid, balanceDue,
  onDiscountChange, onTaxRateChange, onAmountPaidChange,
  readOnly = false,
}: Props) {
  return (
    /* Full-width on mobile, right-aligned fixed-width on sm+ */
    <div className="w-full sm:flex sm:justify-end">
      <div className="w-full sm:w-80 overflow-hidden rounded-2xl border border-card-border bg-card shadow-sm">

        <div className="border-b border-card-border px-5 py-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-muted">Totals</p>
        </div>
        ...
        <div className="px-5 py-4 space-y-0">
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <span className="text-sm font-medium tabular-nums text-foreground">{fmt(subtotal, currency)}</span>
          </div>
          ...
          <div className="flex items-center justify-between py-2">
            ...
            {readOnly ? (
              <span className="text-sm font-medium tabular-nums text-foreground">{fmt(tax, currency)}</span>
            ) : (
              <div className="flex items-center gap-2">
                <div className="flex items-center overflow-hidden rounded-lg border border-card-border bg-background">
                  <input
                    ...
                    placeholder="0"
                    className="w-12 px-2 py-1.5 text-right text-sm tabular-nums focus:outline-none bg-transparent"
                  />
                  <span className="pr-2.5 text-xs text-muted">%</span>
                </div>
          ...
          <div className="flex items-center justify-between border-t border-primary/20 mt-1 pt-3 pb-1">
            ...
            <span className="text-base font-bold text-primary tabular-nums">{fmt(total, currency)}</span>
          </div>
          ...
          {amountPaid !== undefined && (
            <>
              <div className="flex items-center justify-between border-t border-card-border pt-3 pb-2 mt-1">
                ...
                {readOnly ? (
                  <span className="text-sm font-medium tabular-nums text-foreground">{fmt(amountPaid, currency)}</span>
                ) : (
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs text-muted">AUD</span>
                    <input
              ...
              <div className={`flex items-center justify-between rounded-xl px-3 py-2.5 mt-1 ${(balanceDue ?? 0) > 0 ? "bg-danger/5" : "bg-green-500/5"}`}>
                <span className="text-sm font-bold text-foreground">Balance Due</span>
                <span className={`text-base font-bold tabular-nums ${(balanceDue ?? 0) > 0 ? "text-danger" : "text-green-600"}`}>
