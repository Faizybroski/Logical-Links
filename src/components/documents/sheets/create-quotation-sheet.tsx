...
export function CreateQuotationSheet({ open, onClose, loadId }: CreateQuotationSheetProps) {
        ...
        <div className="flex items-center justify-between border-b border-card-border px-6 py-4 flex-shrink-0">
          <div>
            <h2 className="text-lg font-bold text-foreground">
              {loadPrefill ? `New Quotation for ${loadPrefill.loadNumber}` : "New Quotation"}
            </h2>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-6 py-6">
            {loadId && deliveryLoading ? (
              ...
              // Keyed by loadId: this Sheet's children stay mounted even
              // while closed, so without a key change here, creating a
              // quotation for a different delivery right after another
              // (without a full page reload) would reuse the first
              // delivery's stale prefill data.
              <QuotationEditor
                key={loadId ?? "new"}
                profileId={user.id}
