...
const BASE_NAVIGATION = [
  ...
  { label: "My Deliveries", href: "/corporate/deliveries", icon: Truck },
  { label: "Invoices", href: "/corporate/invoices", icon: FileText },
  { label: "Quotations", href: "/corporate/quotations", icon: FileQuestion },
  { label: "Alerts", href: "/corporate/notifications", icon: Bell },
  { label: "Support", href: "/corporate/support", icon: LifeBuoy },
  { label: "Company", href: "/corporate/company", icon: Building2 },
  { label: "My Profile", href: "/corporate/profile", icon: User },
];
...
export default function CorporateSidebar({ isOpen = false, onClose }: Props) {
  ...
  function changeMode(nextMode: SidebarMode) {
    setMode(nextMode);
    localStorage.setItem("corporate-sidebar-mode", nextMode);
  ...
  async function signOut() {
    console.info(
      "[Auth][CorporateSidebar] User initiated logout — user=" +
        (user?.email ?? "unknown"),
    );
    ...
    try {
      ...
      console.warn(
        "[Auth][CorporateSidebar] Backend logout request failed:",
        err,
      );
    }
