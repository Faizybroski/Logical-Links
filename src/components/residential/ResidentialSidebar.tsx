...
export default function ResidentialSidebar({ isOpen = false, onClose }: Props) {
  ...
  function changeMode(nextMode: SidebarMode) {
    setMode(nextMode);
    localStorage.setItem("residential-sidebar-mode", nextMode);
  ...
  async function signOut() {
    try {
      ...
      console.warn("[Auth][ResidentialSidebar] Backend logout request failed:", err);
    }
        ...
        {isExpanded ? (
          <div className="flex items-center gap-2.5 rounded-xl border border-white/5 bg-sidebar-secondary p-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <span className="text-[10px] text-primary">Residential</span>
            </div>
          </div>
        ) : (
