...
export function getCreatorName(shipment: Shipment): string {
  ...
  return p.role === "admin" ? "Admin User" : "Shipper User";
}
...
/**
 * Displays a colour-coded role chip (Admin / Shipper) with an optional creator name.
 *
 * Admin   → blue shield  — "Admin-Created"
 * Shipper → violet truck — "Shipper-Owned"
...
export function CreatorBadge({ shipment, size = "md", showName = false }: CreatorBadgeProps) {
  ...
  if (isShipper) {
    return (
      <span className={`inline-flex items-center gap-1 rounded-full border border-violet-200 bg-violet-50 font-medium text-violet-700 dark:border-violet-700 dark:bg-violet-950 dark:text-violet-300 ${padSize} ${textSize}`}>
        ...
        {showName ? `Shipper: ${creatorName}` : "Shipper-Owned"}
      </span>
    );
  }
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border border-primary-dark bg-primary-light/50 font-medium text-primary-dark dark:border-primary-700 dark:bg-primary-950 dark:text-primary-300 ${padSize} ${textSize}`}>
      ...
      {showName ? `Admin: ${creatorName}` : "Admin-Created"}
    </span>
  );
}
