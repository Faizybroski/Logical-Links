...
import type { AccountProfile, Shipment } from "@/types/api.types";

import { FormSection, LoadLocationFields, LoadCargoFields, LoadScheduleFields, LAST_MILE_SERVICE_TYPES, F } from "./load-form-fields";
...
interface CreateLoadSheetProps {
  ...
  /** Locks this load to a corporate customer (account) — hides the company picker. */
  presetAccountId?: string;
}

export function CreateLoadSheet({ open, onClose, context, presetCustomerId, presetAccountId }: CreateLoadSheetProps) {
  ...
  const form = useForm<LoadFormValues>({
    ...
    defaultValues: {
      shipmentType:        context === "residential" ? "last_mile" : "freight",
  ...
  const showServiceType = context === "residential" || (context === "corporate" && shipmentType === "last_mile");
  ...
  async function onSubmit(values: LoadFormValues) {
    try {
      const created = await createMut.mutateAsync({
        ...
        serviceType:         values.shipmentType === "last_mile" ? values.serviceType : undefined,
        serviceLevel:        values.shipmentType === "last_mile" ? values.serviceLevel : undefined,
      ...
      if (assignedDriverId) {
        try {
          await api.post<ApiResponse<Shipment>>(
            `/api/v1/shipments/${created.data.shipment_id}/assign-driver`,
            { employeeId: assignedDriverId },
          );
        } catch (err) {
          toast.error(`Load created, but assigning the driver failed: ${(err as Error).message}`);
          onClose();
          return;
        }
      }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex h-full flex-col">
          ...
          <div className="flex items-center justify-between border-b border-card-border px-6 py-4 flex-shrink-0">
            <div>
              <h2 className="text-lg font-bold text-foreground">
                {context === "residential" ? "Create a Delivery" : context === "corporate" ? "Create a Load" : "Create New Load"}
              </h2>
              <p className="mt-0.5 text-xs text-muted">
                {context === "residential"
                  ? "Submit a new last-mile delivery for this customer"
                  : context === "corporate"
                    ? "Submit a new load for this corporate customer"
          ...
          <div className="flex items-center justify-end gap-3 border-t border-card-border px-6 py-4 flex-shrink-0">
            ...
            >
              {saving ? "Creating…" : context === "residential" ? "Create Delivery" : "Create Load"}
            </Button>
          </div>
        </form>
      </Form>
