...
export default function ShipperQuotationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id }  = use(params);
  ...
  if (!quotation) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4">
        ...
        <Link href="/shipper/quotations" className="text-sm text-primary underline">Back to quotations</Link>
      </div>
    );
  }
  ...
  async function handleConvert() {
    try {
      ...
      if (newId) router.push(`/shipper/invoices/${newId}`);
    } catch (e) { toast.error((e as Error).message); }
  }
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            ...
            <PricingSummary
              ...
              taxRate={quotation.tax_rate} tax={quotation.tax} total={quotation.total} readOnly
            />
          </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-card-border bg-card shadow-sm">
              ...
              <div className="space-y-0">
                {[
                  ...
                  { label: "Subtotal",   value: new Intl.NumberFormat("en-AU", { style: "currency", currency: "AUD" }).format(quotation.subtotal), mono: true },
                ...
                <div className="flex items-center justify-between bg-primary/5 px-5 py-4">
                  ...
                  <span className="text-base font-bold text-primary tabular-nums">
                    {new Intl.NumberFormat("en-AU", { style: "currency", currency: "AUD" }).format(quotation.total)}
                  </span>
                </div>
              </div>
            </div>
