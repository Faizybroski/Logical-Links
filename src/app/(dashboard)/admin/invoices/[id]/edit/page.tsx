  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="sticky top-0 z-20 border-b border-card-border bg-card/95 backdrop-blur-xl">
        <div className="mx-auto max-w-6xl px-4 py-4 sm:px-6">
          <nav className="mb-3 flex items-center gap-1.5 text-xs text-muted">
            <Link href="/admin/invoices" className="hover:text-foreground transition-colors">Invoices</Link>
            <ChevronRight className="h-3 w-3" />
            <Link href={`/admin/invoices/${id}`} className="hover:text-foreground transition-colors">{invoice.invoice_number}</Link>
            <ChevronRight className="h-3 w-3" />
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-2 sm:py-8">
        <InvoiceEditor profileId={user.id} invoice={invoice} redirectTo={`/admin/invoices/${id}`} isAdmin />
      </div>
    </div>
  );
}
